...
!############################################################################
!
subroutine VISU_INSTB (ux1,uy1,uz1,phi1,ta1,tb1,tc1,td1,te1,tf1,tg1,th1,ti1,di1,&
...
do k=1,xsize(3)
   do j=1,xsize(2)
      do i=1,xsize(1)
         uvisu=uvisu+(tf1(i,j,k)-th1(i,j,k))**2 &
                    +(tg1(i,j,k)-tc1(i,j,k))**2 &
                    +(tb1(i,j,k)-td1(i,j,k))**2
      enddo
   enddo
enddo
call MPI_ALLREDUCE(uvisu,enstrophy,1,real_type,MPI_SUM,MPI_COMM_WORLD,code)
enstrophy=0.5_mytype*enstrophy/(nx*ny*nz)
if (nrank.eq.0) write(*,*) 'ENSTROPHY ',t,enstrophy
!############################################################################
...
do k=1,xsize(3)
   do j=1,xsize(2)
      do i=1,xsize(1)
         uvisu=uvisu+ux1(i,j,k)**2+uy1(i,j,k)**2+uz1(i,j,k)**2
      enddo
   enddo
enddo
call MPI_ALLREDUCE(uvisu,eek,1,real_type,MPI_SUM,MPI_COMM_WORLD,code)
eek=0.5_mytype*eek/(nx*ny*nz)
if (nrank.eq.0) write(*,*) 'KINETIC ENERGY ',t,eek
!############################################################################
end subroutine VISU_INSTB

!############################################################################
!
subroutine VISU_INSTA (ux1,uy1,uz1,rho1,temperature1,phi1,ta1,tb1,tc1,td1,te1,tf1,tg1,th1,ti1,di1,&
     ta2,tb2,tc2,td2,te2,tf2,tg2,th2,ti2,tj2,di2,&
     ta3,tb3,tc3,td3,te3,tf3,tg3,th3,ti3,di3,phG,uvisu)
!
!############################################################################
